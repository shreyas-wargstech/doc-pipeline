...
Next step: complete the pipeline LOCAL-FIRST — **build `nas/uploader/` first** (sub-project A: PDF→pages→S3→manifest→trigger; unblocks real end-to-end runs), then `cloud/structure/` → match stage → `cloud/persist/`, then AWS infra LAST. Full roadmap + rationale in auto-memory `pipeline-completion-roadmap.md` and session_log 2026-06-07. (Was: implement cloud/structure/ — re-sequenced because uploader is the real blocker: no PDF can enter the system today.)
