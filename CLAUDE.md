...
Open threads: wire GCV creds (+ run skipped GCV integration test); wire OPENROUTER_API_KEY (skipped openrouter integration test); calibrate triage + preprocess + match fuzzy thresholds (`FUZZY_MATCH_HIGH`/`FUZZY_REVIEW_LOW`) on real data (all uncalibrated, no labeled match pairs yet). DONE 2026-06-08: match integration tests run live on real Postgres (3/3 green). DONE 2026-06-06: refreshed stale docs + implemented cloud/classifier/llm.py (OpenRouter, same key as T3, 14 unit tests green, 88 total).
