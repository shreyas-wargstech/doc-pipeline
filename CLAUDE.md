...
- Page-typer cost guard (2026-06-15, FIX-047): empty-OCR pages short-circuit to `("blank", 0.9)` — never pay a VLM classify to look at a blank; and `invoice`/`letter_body` got keyword rules (were uncovered → always escalated). On the 13-page bundle `ocr_classify` was the dominant spend (not transcription); these cut the avoidable escalations. Anchors UNCALIBRATED — tune via eval lab.
