...
**Root cause:** `StructureFunction` had `Timeout: 30`, too tight for the structure text LLM (`openrouter/free`, the intentional cost-saving model for text-only jobs, which is legitimately slow). When a call took >30s the Lambda timed out (`Status: timeout` at exactly 30000ms, only `structure_llm_requesting` logged), SQS redelivered, and after `maxReceiveCount` the message dead-lettered → doc never advanced to match. Flaky: runs that squeaked under 30s succeeded, which is why earlier smoke tests sometimes reached `matched` and sometimes `unmatched`/stuck.

**Fix (template, FIX-075):** bumped `StructureFunction` `Timeout` 30→120 (kept ≤ `StructureQueue` VisibilityTimeout of 120 so SQS doesn't redeliver a still-running message). Also added an `OpenRouterTextModel` param (default `openrouter/free`) wired to `OPENROUTER_TEXT_MODEL` in `Globals.Function.Environment` so the text model is explicit/tunable per deploy — but **free is the intended value for text**; `google/gemini-2.5-flash` is reserved for the VLM (`OpenRouterModel`) only.

> **Correction (same day):** the first pass also swapped the text model to `google/gemini-2.5-flash` (both live env and as the param default), on the theory that the free model was the problem. The user clarified that `openrouter/free` is fine for text-based LLM jobs and gemini-2.5-flash should be VLM-only — the **timeout was the actual fix**. Reverted: live env on Structure+Ocr back to `openrouter/free`; param default back to `openrouter/free`. Lesson recorded in the rules below.

**Verify:** with `Timeout: 120` the free text model completes within budget; the test doc reached `match_status=matched`, `index_status=done`, `registration_no=92008` end-to-end.
...
- A slow LLM behind a Lambda is a **timeout** problem, not necessarily a model problem. `openrouter/free` is the intended model for text-only jobs (classifier + structure) for cost; size the Lambda `Timeout` for its latency rather than swapping to a paid model. `google/gemini-2.5-flash` is for the VLM only.
